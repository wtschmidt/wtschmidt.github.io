...
_.first = function(array, value){
    if (!Array.isArray(array)){
        return [];
    //checking if value is a number and defined and return first element of the array  
    } else if (typeof value !== 'number' || value === undefined){
        return array[0];
    //check to see if value is negative to return an empty array
    } else if (value < 0){
        return [];
    //check if value is bigger than the array length
...
//         //call function once for each property with arguments: value, key, collection
...
// collection can be array or object, will be put through func (function)
// console.log of element/ property values
_.each = function (collection, func){
    if (Array.isArray(collection)){
...
_.map = function(collection, func){
    // original code using for and for-in loops
    // let arr = []
    // if (Array.isArray(collection)){
    //     for (let i = 0; i < collection.length; i++){
    ...
    // }
    // return arr
    // using the _.each function in tandem
    
    const result = [];
    _.each(collection, function(element, index, c){
